...
def create_password_hash(password, salt_hex=None):
    if salt_hex is None:
...
def verify_password(password, salt_hex, expected_hash):
    _, candidate_hash = create_password_hash(
        password,
        salt_hex,
    )
...
def set_authenticated_account(name, email):
    st.session_state.authenticated = True
    st.session_state.account_name = name.strip()
    st.session_state.account_email = normalize_email(email)
    st.session_state.account_role = "Wallet Intelligence"
...
def render_login_page():
    ...
    with left:
        st.html(
            ...
            <div class="login-brand-card">
                ...
                <p>
                    Customer behavior analytics for detecting silent
                    attrition, understanding wallet migration and
                    prioritizing high-value recovery opportunities.
                </p>

                <div class="login-feature">
                    <span>01</span>
                    <div>Share-of-Wallet Intelligence</div>
                </div>

                <div class="login-feature">
                    <span>02</span>
                    <div>Behavioral Customer Segmentation</div>
                </div>

                <div class="login-feature">
                    <span>03</span>
                    <div>Opportunity Prioritization</div>
                </div>

                <div class="login-feature">
                    <span>04</span>
                    <div>Recovery Scenario Simulation</div>
                </div>
            </div>
            """
        )
    ...
    with right:
        st.markdown(
            ...
            <div class="login-form-wrap">
                ...
                <div class="login-subtitle">
                    Create a demo account or sign in to continue.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        create_tab, sign_in_tab = st.tabs(
            [
                "Create account",
                "Sign in",
            ]
        ...
        with create_tab:
            ...
            if create_submitted:
                ...
                if len(name.strip()) < 2:
                    ...
                    normalized_email
                    in st.session_state.demo_accounts
                ):
                    st.warning(
                        "A demo account with this email already exists "
                        "in this browser session. Use Sign in."
                    )
        ...
        with sign_in_tab:
            ...
            if submitted:
                ...
                if not is_valid_email(
                    ...
                    st.error(
                        "No demo account exists for this email "
                        "in the current session. Create an account first."
                    )
        ...
        st.markdown(
            ...
            <div class="auth-demo-note">
                ...
                Accounts are created only for the current Streamlit
                browser session. Passwords are hashed in memory and are
                not written to the project files or customer dataset.
            </div>

            <div class="login-security">
                Customer identifiers displayed in the platform are
                anonymized.
            </div>
            """,
            unsafe_allow_html=True,
